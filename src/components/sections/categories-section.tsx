              ...
              <Card className="group overflow-hidden hover:shadow-lg transition-all cursor-pointer h-full">
                <div className="relative h-40 overflow-hidden">
                  ...
                  <div className="absolute bottom-3 left-3">
                    <h3 className="text-white font-semibold text-lg">{cat.name[locale]}</h3>
                    <p className="text-white/70 text-sm">{cat.itemCount} {t('items')}</p>
                  </div>
                </div>
                <CardContent className="p-3"><Button variant="ghost" size="sm" className="w-full">{t('browse')}</Button></CardContent>
              </Card>
            </Link>
          ))}
        </div>
